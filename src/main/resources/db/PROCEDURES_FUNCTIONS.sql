...
CREATE OR REPLACE PROCEDURE PRC_TAO_HOA_DON(
    ...
    P_TIENCOC IN HOA_DON.TIEN_COC%type
)
AS
BEGIN
    ...
    VALUES (P_MAHD, P_MAKH, P_MANV, NVL(P_TIENCOC, 0), NVL(P_GIAMGIA, 0),
...
BEGIN
    ...
    SAVEPOINT SP_DAT_SAN;CREATE OR REPLACE PROCEDURE PRC_DAT_SAN(
    P_MAHD           IN HOA_DON.MAHD%TYPE,
    P_MACT_THUE_SAN  IN CHI_TIET_HOA_DON_THUE_SAN.MACT_THUE_SAN%TYPE,
    P_MAKH           IN KHACH_HANG.MAKH%TYPE,
    P_MANV           IN NHAN_VIEN.MANV%TYPE,
...
BEGIN
    ...
    IF P_LA_DAT_TRUOC = 1 THEN
        V_TIEN_COC_MOI := ROUND(V_TONG_TIEN_SAN * 0.7, 0);
    ELSE
        V_TIEN_COC_MOI := 0;
    END IF;
