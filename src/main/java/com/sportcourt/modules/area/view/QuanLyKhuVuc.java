...
public class QuanLyKhuVuc extends JPanel {
    ...
    private void loadKhuVucData(String keyword) {
        ...
        SwingWorker<List<KhuVuc>, Void> worker = new SwingWorker<>() {
            @Override
            protected List<KhuVuc> doInBackground() {
                return khuVucController.getKhuVucList(keyword);
            }
    ...
    private void renderTableData(List<KhuVuc> khuVucs) {
        ...
        if (khuVucs.isEmpty()) {
            ...
            for (KhuVuc khuVuc : khuVucs) {
                tablePanel.add(createDataRow(khuVuc, rowIndex++));
            }
        }

        infoLabel.setText("Hiển thị " + khuVucs.size() + " khu vực");
    ...
    private JPanel createDataRow(KhuVuc khuVuc, int rowIndex) {
        ...
        JLabel maKvLabel = new JLabel(khuVuc.maKv());
        ...
        row.add(createAlignedCellPanel(createCellLabel(khuVuc.maCn(), new Color(37, 99, 235)), 20, rowBackground));
        row.add(createAlignedCellPanel(createCellLabel(khuVuc.tenTheThao(), new Color(75, 85, 99)), 20, rowBackground));
        row.add(createAlignedCellPanel(createCellLabel(String.valueOf(khuVuc.soLuongSan()), new Color(17, 24, 39)), 25, rowBackground));
        row.add(createAlignedCellPanel(createCellLabel(formatDate(khuVuc.createdAt()), new Color(75, 85, 99)), 15, rowBackground));
    ...
    private void confirmDelete(KhuVuc khuVuc) {
        ...
        try {
            khuVucController.deleteKhuVuc(khuVuc.maKv());
            loadKhuVucData(searchField.getText());
            JOptionPane.showMessageDialog(this, "Đã xóa khu vực " + khuVuc.maKv() + ".", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
