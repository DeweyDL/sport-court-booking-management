...
// Màn sửa khu vực tách riêng khỏi màn chi tiết để chế độ xem và sửa rõ ràng hơn.
public class SuaKhuVuc extends JPanel {
    ...
    private final JLabel titleLabel = new JLabel("SỬA THÔNG TIN KHU VỰC");
    private final JLabel infoHintLabel = new JLabel("Cập nhật thông tin khu vực và danh sách sân con.");
    ...
    private final Set<String> deletedSanConIds = new LinkedHashSet<>();

    public SuaKhuVuc(KhuVucController khuVucController, Runnable onBackToList, Consumer<String> onSaved) {
    ...
    public void showEditor(String maKv) {
        currentMaKv = maKv;
        selectedImagePath = null;
        deletedSanConIds.clear();
        newSanCons.clear();
    ...
    private JPanel createBasicInfoCard() {
        ...
        // Đã giảm viền trên/dưới xuống 16
        ...
        infoHintLabel.setBorder(new EmptyBorder(6, 0, 14, 0)); // Thu hẹp margin
        ...
        uploadButton.setPreferredSize(new Dimension(300, 44)); // Giảm chiều cao
        ...
        JButton saveButton = createPillButton("LƯU THAY ĐỔI", new Color(57, 255, 20), new Color(26, 42, 8), true);
        saveButton.setPreferredSize(new Dimension(300, 48)); // Giảm chiều cao
        ...
        // Giảm khoảng cách (Vertical Strut) giữa các element
    ...
    private JPanel createEditableField(String labelText, JComponent editor) {
        ...
        // Giảm kích thước khung xuống 74
    ...
    private JPanel createSanConSection() {
        ...
        JLabel subtitle = new JLabel("Thêm, xem chi tiết hoặc xóa sân con trước khi lưu.");
    ...
    private void bindLoaiTheThao(String selectedMaTt) {
        ...
        for (int i = 0; i < model.getSize(); i++) {
            LoaiTheThao loaiTheThao = model.getElementAt(i);
            if (loaiTheThao != null && loaiTheThao.maTt().equals(selectedMaTt)) {
                sportTypeComboBox.setSelectedIndex(i);
                break;
            }
        }
    }

    private void bindKhuVuc(KhuVuc khuVuc) {
        titleLabel.setText("SỬA THÔNG TIN KHU VỰC - " + khuVuc.maKv());
        infoHintLabel.setText("Cập nhật thông tin cơ bản và danh sách sân con của khu vực.");
        updateCourtCountLabel();
    }

    private void renderSanConTable() {
        tableBodyPanel.removeAll();
        List<SanCon> visibleSanCons = getVisibleSanCons();

        if (visibleSanCons.isEmpty()) {
            tableBodyPanel.add(createEmptyRow());
        } else {
            for (SanCon sanCon : visibleSanCons) {
                tableBodyPanel.add(createSanConRow(sanCon));
            }
        }

        footerLabel.setText("Đang hiển thị " + visibleSanCons.size() + " sân");
    ...
    private JPanel createSanConRow(SanCon sanCon) {
        ...
        deleteButton.addActionListener(event -> removeSanCon(sanCon));
    ...
    private void openAddSanConDialog() {
        ...
        if (isDuplicateMaSan(maSan)) {
            JOptionPane.showMessageDialog(this, "Mã sân đã tồn tại trong danh sách hiện tại.", "Thông báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    private boolean isDuplicateMaSan(String maSan) {
        for (SanCon sanCon : currentSanCons) {
            if (!deletedSanConIds.contains(sanCon.maSan()) && sanCon.maSan().equalsIgnoreCase(maSan)) {
                return true;
            }
        }
    ...
    private void removeSanCon(SanCon sanCon) {
        int confirm = JOptionPane.showConfirmDialog(
                this,
                "Bạn chắc chắn muốn xóa sân con " + sanCon.maSan() + " không?",
                "Xác nhận xóa",
        ...
        if (sanCon.createdAt() == null) {
            newSanCons.removeIf(item -> item.maSan().equalsIgnoreCase(sanCon.maSan()));
        } else {
            deletedSanConIds.add(sanCon.maSan());
        }
        updateCourtCountLabel();
        renderSanConTable();
    }

    private List<SanCon> getVisibleSanCons() {
        List<SanCon> visibleSanCons = new ArrayList<>();
        for (SanCon sanCon : currentSanCons) {
            if (!deletedSanConIds.contains(sanCon.maSan())) {
                visibleSanCons.add(sanCon);
            }
        }
        for (SanConDraft sanConDraft : newSanCons) {
            visibleSanCons.add(new SanCon(
                    sanConDraft.maSan(),
                    currentMaKv,
                    sanConDraft.trangThai(),
                    null
            ));
        }
        return visibleSanCons;
    }

    private void updateCourtCountLabel() {
        courtCountValueLabel.setText(String.valueOf(getVisibleSanCons().size()));
    }
    ...
    private void saveChanges() {
        ...
        KhuVucUpdateRequest request = new KhuVucUpdateRequest(
                currentMaKv,
                selectedLoaiTheThao.maTt(),
                getVisibleSanCons().size(),
                List.copyOf(newSanCons),
                List.copyOf(deletedSanConIds)
        );

        try {
            khuVucController.saveKhuVucChanges(request);
            if (selectedImagePath != null) {
                khuVucController.saveAreaImage(currentMaKv, selectedImagePath);
            }
            JOptionPane.showMessageDialog(this, "Đã lưu thay đổi khu vực.", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
    ...
    private void loadAreaImage(String maKv) {
        Optional<Path> imagePath = khuVucController.getAreaImagePath(maKv);
        imagePath.ifPresentOrElse(imagePreviewPanel::setImage, imagePreviewPanel::clearImage);
    }
