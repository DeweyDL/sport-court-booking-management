...
public class BookingHistoryMockDemo extends JPanel {
    ...
    private void loadBannerImage() {
        ...
        for (String name : names) {
            URL url = BookingHistoryMockDemo.class.getResource(name);
            if (url != null) {
    ...
    private JComponent createImageLabel(String fileName, int width, int height) {
        URL imgUrl = BookingHistoryMockDemo.class.getResource("/image/" + fileName);
        if (imgUrl != null) {
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            JFrame frame = new JFrame("RENTSTA – Lịch Sử Đặt Sân (Demo Ảo)");
            frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            frame.setSize(1200, 900);
            frame.setLocationRelativeTo(null);
            frame.add(new BookingHistoryMockDemo());
