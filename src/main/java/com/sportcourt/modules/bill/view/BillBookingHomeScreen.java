...
public class BillBookingHomeScreen extends JPanel {
    ...
    private static final Color PANEL_BG = new Color(243, 243, 246);
    private static final Color PANEL_DARK = new Color(60, 75, 53);
    private static final Color BORDER = new Color(228, 228, 231);
    private static final Color TEXT_DARK = new Color(26, 28, 30);
    private static final Color BRAND_DARK = new Color(15, 23, 42);
    private static final Color TEXT_MUTED = new Color(113, 113, 122);
    private static final Color TEXT_OLIVE = new Color(60, 75, 53);
    private static final Color GREEN = new Color(57, 255, 20);
    private static final Color GREEN_DARK = new Color(16, 110, 0);
    private static final Color TAG_BLUE_BG = new Color(192, 235, 255, 90);
    private static final Color TAG_BLUE_TEXT = new Color(0, 97, 171);
    private static final Color SOFT_GRAY = new Color(232, 232, 234);

    private static int s(int value) {
        return com.sportcourt.common.style.UIScale.scale(value);
    }
    ...
    private static void prepareText(JComponent component, Font font, Color color) {
    ...
    private static JButton pillButton(String text, Color background, Color foreground) {
    ...
    private static Icon icon(String resourcePath, int width, int height) {
        java.net.URL url = BillBookingHomeScreen.class.getResource(resourcePath);
        if (url == null) {
            return null;
        }
    ...
    private static Image loadImage(String... resourcePaths) {
        for (String path : resourcePaths) {
            java.net.URL url = BillBookingHomeScreen.class.getResource(path);
            if (url != null) {
    ...
    private static JScrollPane cleanScrollPane(Component view) {
    ...
    public BillBookingHomeScreen(CustomerBookingController controller,
                                 BookingRequestHandler onBookingRequested) {
        this.controller = controller;
        this.onBookingRequested = onBookingRequested;
        AppFonts.register();

        // Auto-detect branch for staff or manager
        try {
            com.sportcourt.modules.auth.dto.UserSession session = com.sportcourt.modules.auth.service.SessionManager.requireSession();
            boolean isStaffOrManager = session.getRoleGroups().contains(com.sportcourt.modules.auth.dto.RoleGroupId.BRANCH_MANAGER)
                    || session.getRoleGroups().contains(com.sportcourt.modules.auth.dto.RoleGroupId.CASHIER);
            if (isStaffOrManager && session.getBranchId() != null) {
                String staffBranchId = session.getBranchId().trim();
                List<BranchOption> branches = controller.loadBranches();
                for (BranchOption branch : branches) {
                    if (branch.branchId().trim().equalsIgnoreCase(staffBranchId)) {
        ...
        if (selectedBranch != null) {
            renderEmptyState("Nhấn đặt sân để chọn loại thể thao");
        } else {
    ...
    private JComponent buildFilterCard() {
        ...
        String filterText = "Chưa chọn chi nhánh và loại thể thao";
        if (selectedBranch != null) {
            filterText = selectedBranch.branchName() + " - " + selectedBranch.address() + " | Chưa chọn loại thể thao";
        }
        filterValueLabel = label(filterText, bold(15f), TEXT_DARK);
        ...
        String btnText = (selectedBranch != null) ? "Chọn môn & ngày" : "Chọn chi nhánh";
        JButton selectFilterButton = pillButton(btnText, GREEN, GREEN_DARK);
            ...
            try {
                ...
                if (isStaffOrManager && session.getBranchId() != null) {
                    ...
                    for (int i = 0; i < branchCombo.getItemCount(); i++) {
                        ...
                        if (item.branchId().trim().equalsIgnoreCase(staffBranchId)) {
