...
final class ProductEditDialog {
    ...
    static ProductUpdateRequest show(Component parent, ProductResponse product) {
        ...
        JTextField txtMaSp = readonly(product.getMaSp());
        JTextField txtTenSp = new JTextField(product.getTenSp());
        JTextField txtDvt = new JTextField(product.getDvt());
        JTextField txtGia = new JTextField(product.getGia() == null ? "" : moneyFormat.format(product.getGia()));
        installMoneyFormatter(txtGia, moneyFormat);
        JTextField txtSlTon = readonly(product.getSlTon() == null ? "0" : String.valueOf(product.getSlTon()));
        ...
        addField(form, g, 4, "Số lượng tồn kho", txtSlTon);
        ...
        btnSave.addActionListener(e -> {
            ...
            req.setMaSp(product.getMaSp());
            req.setTenSp(tenSp);
            req.setDvt(dvt);
            req.setGia(gia);
            req.setSlTon(product.getSlTon() == null ? 0 : product.getSlTon());
            result[0] = req;
            dialog.dispose();
        });
