...
public class CostAdd extends JPanel {
    ...
    public CostAdd(CostController costController, Consumer<String> onSaved) {
    ...
    private void prepareCreateForm() {
        generatedMaBg = costController.generateNextMaBg();
    ...
    private void bindKhungGio() {
        ...
        if (options == null || options.isEmpty()) {
            for (int h = 0; h <= 23; h++) {
                model.addElement(new Cost.KhungGioOption(null, h, h + 1));
            }
        } else {
    ...
    private void saveNewBangGia() {
        ...
        try {
            costController.createBangGia(request);
