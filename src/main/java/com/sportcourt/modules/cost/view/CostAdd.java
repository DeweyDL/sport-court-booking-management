...
final class CostAdd {
    ...
    static boolean show(Component parent, Store store) {
        String generatedMaBg = store.generateNextMaBg();

        Window owner = parent == null ? null : SwingUtilities.getWindowAncestor(parent);
        JDialog dialog = new JDialog(owner, "Thêm bảng giá", Dialog.ModalityType.APPLICATION_MODAL);
        ...
        JLabel title = new JLabel("Thêm bảng giá mới");
        title.setFont(AppFonts.lexendBold(24f));
        title.setForeground(TEXT_DARK);
        JLabel subtitle = new JLabel("Khởi tạo thông tin khung giờ và giá.");
        ...
        JTextField maBgField = readonly(generatedMaBg);
        JComboBox<AreaOption> areaCombo = new JComboBox<>(store.getAreaOptions().toArray(new AreaOption[0]));
        java.util.List<KhungGioOption> khungGioList = store.getKhungGioOptions();
        if (khungGioList == null || khungGioList.isEmpty()) {
            khungGioList = new java.util.ArrayList<>();
            for (int h = 0; h <= 23; h++) {
        ...
        JButton btnSave = button("Thêm bảng giá", BRAND_GREEN, Color.WHITE);
        ...
        btnSave.addActionListener(e -> {
            ...
            try {
                store.create(new CostItem(generatedMaBg, selectedArea.maKv(),
                ...
                JOptionPane.showMessageDialog(dialog,
                        ...
                        "Lỗi thêm bảng giá", JOptionPane.ERROR_MESSAGE);
            }
        });
